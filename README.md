...
### Hypothesis, answered
*SNARL qPCR more often fails to detect Bd at low quantities than RZ qPCR.* **Moderately supported, not conclusive.** On the same extract, SNARL qPCR has a 92% posterior probability of producing more non-detects (odds of a zero ≈ 2.3× RZ's, 95% CI 0.8 to 7.6), and needs ≈ 0.45 log10 more Bd for 50% detection (95% CI −0.15 to 1.30). Several low-load extracts were detected by RZ but not SNARL (experiment panel A). Among positive results, SNARL qPCR quantifies the same as RZ (no constant or load-dependent quantity bias). The larger detection gap between the labs' standard pipelines comes from PrepMan extraction.
...
2. **qPCR step**: no significant difference in positive quantities; probably more non-detects at SNARL at low loads (see Hypothesis, answered).
...
## Hypothesis
qPCR results from the SNARL lab more often fail to detect Bd at low quantities compared to the RZ qPCR protocol.
...
2. **Model 2: Experiment**. brms non-linear hurdle-lognormal. For positive results, expected log load `mu = L + d + s · SNARL_qPCR · (L − L_ref)`:
   ...
   - `s`: SNARL qPCR slope deviation; SNARL − RZ = `d_snarl + s·(L − L_ref)`. `s > 0` = SNARL increasingly under-quantifies below `L_ref`; `s = 0` = constant bias. This tests a load-dependent *quantity* bias among positives, separate from the detection hypothesis (which is tested by the SNARL qPCR effect on `hu`). `L_ref` = median positive load in the experiment.
   - `hu` (detection): intercepts by extraction method and qPCR lab (the SNARL qPCR `hu` effect is the test of the hypothesis), plus a frog-level intercept correlated with the frog-level intercept in `L` (correlation approach, as in Model 1). Labs differ in baseline detection but share the detection-vs-load slope; explicit lab-specific detection curves were not used.
...
- `rz_snarl_qpcr_model2_experiment.qmd`: Model 2 (experiment). Reads the Model 1 export (render Model 1 first; warns if Model 1 was fit on an older data import). Model data, priors from Model 1, structure, fit, summary, lab differences table (log10 biases, slope, detection, noise), divergence pairs plot, SNARL − RZ qPCR difference across load and detection by pipeline, posterior predictive checks, Model 2b (constant bias) fit and LOO comparison with constant correction table, detection conversion (P(detected) vs true load by pipeline, 50%/90% detection thresholds, exported to `fits/m2b_detection_export.rds`), correction to the Qiagen–RZ scale under Model 2 (plot, table, exported draws).

- `rz_snarl_qpcr_summary.qmd`: plain-language summary for the team (goals, experiment, raw data, findings, conversion rule, limitations, discussion points), with collapsible technical details. Renders to a single self-contained HTML file. Uses saved fits and exports only (render Model 1 and Model 2 first); nothing is refit.
